...
# The following script analyse the fragmentation profiles of cfDNA molecules of all dataset using cfDNApro, with each group of data being combined.  It requires the insert size metrics text files from Picard Collectinsertsize, and each group should be stored in the respective folder within the data_path.  The name of the folder would be the group included in the analysis.  Using the tibble data structure available by the tools, we also analysis the sample stastics and present them as a floating layer ontop of the output.
...
order <- c("PTCL_combined", "healthy_control", "PBMC_combined")
...
# Finalize plots.
suppressWarnings(
  median_grps<-ggpubr::ggarrange(p1http://127.0.0.1:28803/graphics/41aae3c7-2d69-433d-a3d9-79b11f3880f8.png,
                                 p2,
                                 label.x = 0.3,
                                 ncol = 1,
                                 nrow = 2
...
median_grps
ggsave("median_grps_combined.png",plot = last_plot())


#perform Two-sample Kolmogorov-Smirnov test
data1 <- compare_grps$median_prop_plot$data %>% group_by(group) %>%  filter(cur_group()$group == "PTCL_combined")
data2 <- compare_grps$median_prop_plot$data %>% group_by(group) %>%  filter(cur_group()$group == "healthy_control")
